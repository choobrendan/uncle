...
function Graph() {
      ...
      Object.entries(row).forEach(([colName, value]) => {
        ...
        try {
          ...
          if (col.type === "unknown") {
            ...
            col.type !== parsed.type &&
            !(col.type === "number" && parsed.type === "null")
          ) {
          ...
          setErrorMessage("Table Created with Errors");
        ...
        switch (filter.type) {
          case "number":
            if (filter.value.operation === "greaterThan") {
              ...
              return (
                value >= filter.value.value[0] && value <= filter.value.value[1]
              );
    ...
    switch (info.type) {
      case "number":
        ...
        return (
          ...
          >
            <span
              style={{ width: "107px", textAlign: "center", color: "black" }}
            >
              {colName}
            </span>
            <select
              ...
              onChange={(e) =>
                handleNumericOperationChange(colName, e.target.value)
              }
            ...
            >
              <input
                ...
                onChange={(e) =>
                  handleNumericInputChange(colName, "value1", e.target.value)
                }
                step="any"
                onKeyDown={(e) => {
              ...
              {(numInput.operation === "between" ||
                numInput.operation === "notBetween") && (
                <>
            ...
            <button onClick={() => removeColumnFilter(colName)}>Remove</button>

            {filters[colName] && (
              <div style={{ marginLeft: "10px", fontStyle: "italic" }}>
                Active filter:{" "}
                {filters[colName].value.operation === "between" ||
        ...
        return (
          ...
          >
            ...
            <input
              ...
              onChange={(e) =>
                handleFilterChange(colName, "date", [
                  e.target.value,
                  filters[colName]?.value?.[1] ||
                    info.max?.toISOString().split("T")[0],
                ])
              }
              style={{ marginRight: "10px" }}
            />
            ...
            <input
              ...
              onChange={(e) =>
                handleFilterChange(colName, "date", [
                  filters[colName]?.value?.[0] ||
                    info.min?.toISOString().split("T")[0],
                  e.target.value,
                ])
              }
              style={{ marginRight: "10px" }}
            ...
            {filters[colName] && (
              <>
                ...
                <div style={{ fontStyle: "italic" }}>
                  Active filter: From {filters[colName].value[0]} to{" "}
                  {filters[colName].value[1]}
                </div>
              </>
            )}
  ...
  const renderActiveFilters = () => {
    const activeFilters = Object.entries(filters).filter(
      ([colName, filter]) => filter !== null
    );

    return <div className="active-filters"></div>;
  };
  ...
  const renderDataTable = () => {
    ...
    return (
      ...
      >
        ...
        <table
          style={{ color: !simplify ? "aliceblue" : "black" }}
      ...
      <div style={simplifiedStyles.container}>
        {isLoading && csvData.length === 0 && (
          <div style={simplifiedStyles.fileUploader}>
            ...
            />
            {isLoading && file && (
              <div style={simplifiedStyles.loadingMessage}>
        ...
        {!isLoading && csvData.length > 0 && (
          <div style={simplifiedStyles.dataExplorer}>
            <div style={simplifiedStyles.filtersContainer}>
              ...
              <div style={simplifiedStyles.filterSelectorRow}>
                ...
                <button
                  ...
                  disabled={
                    allColumns.length === 0 ||
                    allColumns.length === activeFilterColumns.length
                  }
                  style={{
                    backgroundColor:
                      allColumns.length === 0 ||
              ...
              <div style={{ height: "120px", overflowX: "auto" }}>
                {activeFilterColumns.length === 0 ? (
                  ...
                  activeFilterColumns.map((colName) =>
                    renderColumnFilter(colName)
                  )
                )}
              </div>
            ...
            <div style={simplifiedStyles.dataPreview}>
              ...
              {renderDataTable(true)}{" "}
              {/* Pass a flag to use simplified styling */}
            </div>
          </div>
        )}
      ...
      <div className="graph">
        {isLoading && csvData.length === 0 && (
          <div className="csv-uploader">
            ...
            />
            {isLoading && file && (
              <div style={{ fontSize: `${16 * textSizeModifier}px` }}>
                Loading and analyzing data...
              </div>
            )}
            {errorMessage && (
        ...
        {!isLoading && csvData.length > 0 && (
          <div className="data-explorer">
            ...
            <div
              ...
              style={{ width: "750px", fontFamily: font }}
            >
